...
function Discard_(tableEvents) {
	class Discard extends React.Component {
		constructor(props) {
			super(props)
			this.state = { topCard: this.props.topCard }
			this.cards = [this.state.topCard]
			//TODO: Add tableEvents.off equivalent when component unmounts (at roundEnded)
			tableEvents.on('cardMoved', (table, {from, to}) => {
				if(
					...
					if(
						from.cardIndex == this.cards.length - 1
						|| to.cardIndex == this.cards.length
					) {
						this.setState({topCard: newDiscard[newDiscard.length - 1]})
		...
		render() {
			...
			return <Card rank={topCard.rank} suit={topCard.suit} key={uuidv4()} />
		}
	}
