...
const { getOrder_, sendMessage_, pingMessageListener_ } = require('../../utility/acknowledgement.js')
...
const onConnection = onConnection_({
	...
	callback: (conn) => {
		...
		const pingListener = pingMessageListener_(conn, messageHistory)
		conn.on('message', pingListener)
	}
})
