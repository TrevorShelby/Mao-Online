...
function accuse_(game, seatedActionPools, accuserSeat) {
	...
	function accuse(accusedSeat) {
		seatedActionPools.forEach( (actionPool, poolOwnerSeat) => {
			...
			if(poolOwnerSeat == accuserSeat) {
				actionPool.activate('cancelAccusation')
			}
			else if(poolOwnerSeat == accusedSeat) {
				actionPool.activate('acceptAccusation')
			}
